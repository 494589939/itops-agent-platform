...
router.get('/', (_req, res) => {
  try {
    ...
    logger.error('Failed to get server groups:', error);
...
router.get('/tree', (_req, res) => {
  try {
    ...
    logger.error('Failed to get server group tree:', error);
...
router.post(
  ...
  validateBody(serverGroupSchemas.createGroup),
  (req, res) => {
    try {
...
router.put(
  '/:id',
  validateParams(serverGroupSchemas.groupId),
  validateBody(serverGroupSchemas.updateGroup),
  (req, res) => {
    try {
...
router.delete('/mapping', requireRole('admin', 'operator'), (req, res) => {
  try {
...
router.post(
  ...
  requireRole('admin', 'operator'),
  validateParams(serverGroupSchemas.groupId),
  validateBody(serverGroupSchemas.moveGroup),
  (req, res) => {
    try {
