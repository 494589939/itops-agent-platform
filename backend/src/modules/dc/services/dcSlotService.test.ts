 ...
 *   - assignSlot: 生命周期 to_slot_end 使用 resolvedEndU（Bug1 修复回归）
 *   - moveSlot: 排除自身的冲突检测
 *   - moveSlot: 跨机柜移位 → 写入 moved 生命周期
 *   - moveSlot: 生命周期 device_type 使用设备类型而非 device_type_id（Bug2 修复回归）
 *   - removeSlot: 写入 unmounted 生命周期
 *   - removeSlot: 生命周期 device_type 使用设备类型而非 device_type_id（Bug2 修复回归）
 */
...
describe('dcSlotService', () => {
  ...
  describe('assignSlot', () => {
    ...
    it('Bug1 修复: u_height 自动算 end_u 时，生命周期 to_slot_end 使用 resolvedEndU 而非 input.end_u', async () => {
      // 模拟 device_type_id 对应 2U 高度设备
      mockRepo.devices.getDeviceTypeUHeight.mockReturnValue(2);
      await dcSlotService.assignSlot({
        rack_id: 'r1', device_id: 'd1', device_type: 'server',
        device_type_id: 'dt-1', start_u: 10, end_u: 99, // input.end_u=99，但实际只有 2U
      });
      // slot 表 end_u 应为 resolvedEndU = 10+2-1 = 11
      expect(mockRepo.slots.create).toHaveBeenCalledWith(expect.objectContaining({
        end_u: 11,
      }));
      // 生命周期 to_slot_end 也应为 11（resolvedEndU），而非 input.end_u=99
      expect(mockRepo.devices.createLifecycle).toHaveBeenCalledWith(expect.objectContaining({
        to_slot_end: 11,
      }));
    });

    it('Bug0 回归: device_type 正确写入 slot 表和生命周期', async () => {
      await dcSlotService.assignSlot({
        rack_id: 'r1', device_id: 'd1', device_type: 'network_device',
        start_u: 20, end_u: 21,
      });
      // slot 表 device_type 正确
      expect(mockRepo.slots.create).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('moveSlot', () => {
    it('成功移位（同机柜内）不写生命周期', async () => {
      mockRepo.slots.getById.mockReturnValue({
        ...
        device_type: 'server', start_u: 5, end_u: 6,
      });
    ...
    it('Bug2 修复: 跨机柜移位生命周期 device_type 使用设备类型字符串而非 device_type_id(UUID)', async () => {
      mockRepo.slots.getById.mockReturnValue({
        id: 's1', rack_id: 'r1', device_id: 'd1',
        device_type: 'network_device', device_type_id: 'dt-uuid-1234',
        start_u: 5, end_u: 6,
      });
      await dcSlotService.moveSlot({
        id: 's1', rack_id: 'r2', start_u: 10, end_u: 12,
      });
      // 验证生命周期中 device_type 是 'network_device'（设备类型），而非 'dt-uuid-1234'（UUID）
      expect(mockRepo.devices.createLifecycle).toHaveBeenCalledWith(expect.objectContaining({
        device_type: 'network_device',
      }));
      expect(mockRepo.devices.createLifecycle).not.toHaveBeenCalledWith(expect.objectContaining({
        device_type: 'dt-uuid-1234',
      }));
    });
  ...
  describe('removeSlot', () => {
    ...
    it('Bug2 修复: 移除设备时生命周期 device_type 使用设备类型字符串而非 device_type_id(UUID)', async () => {
      mockRepo.slots.getById.mockReturnValue({
        id: 's1', rack_id: 'r1', device_id: 'd1',
        device_type: 'vm_host', device_type_id: 'dt-uuid-5678',
        start_u: 5, end_u: 6,
      });
      await dcSlotService.removeSlot('s1');
      // 验证生命周期中 device_type 是 'vm_host'（设备类型），而非 'dt-uuid-5678'（UUID）
      expect(mockRepo.devices.createLifecycle).toHaveBeenCalledWith(expect.objectContaining({
        device_type: 'vm_host',
      }));
      expect(mockRepo.devices.createLifecycle).not.toHaveBeenCalledWith(expect.objectContaining({
        device_type: 'dt-uuid-5678',
      }));
    });
