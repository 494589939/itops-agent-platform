 ...
 *
 * 重要：重建顺序必须是 dc_racks → dc_rack_slots → dc_pdus
 *   因为 dc_rack_slots 和 dc_pdus 的 FK 都引用 dc_racks。
 *   如果先建 dc_rack_slots 再建 dc_racks，当 dc_racks 被 RENAME 为 _backup 时，
 *   dc_rack_slots 的 FK 会被 SQLite 自动"污染"指向 _backup 表名。
 */
const v058DcConstraints: Migration = {
  ...
  up: async (db: any) => {
    ...
    // ── 1) dc_racks.room_id FOREIGN KEY 约束（必须先于 dc_rack_slots 重建）──
    ...
    // ── 2) dc_rack_slots.device_type CHECK 约束 + FK 到 dc_racks ──
    //    必须在 dc_racks 重建之后，否则 FK 会指向 _dc_racks_backup
    const slotsHasCheck = (db.prepare(`
      SELECT sql FROM sqlite_master WHERE type='table' AND name='dc_rack_slots'
    `).get() as { sql: string } | undefined)?.sql || '';
    const slotsRefsBackup = slotsHasCheck.includes('_dc_racks_backup');
    if (!slotsHasCheck.includes('CHECK') || slotsRefsBackup) {
      if (slotsRefsBackup) {
        logger.warn('  ⚠ dc_rack_slots FK references _dc_racks_backup (corrupted), fixing...');
      } else {
    ...
    `).get() as { sql: string } | undefined)?.sql || '';
    const pdusRefsBackup = pdusHasFk.includes('_dc_racks_backup');
    if (!pdusHasFk.toUpperCase().includes('REFERENCES DC_RACKS') || pdusRefsBackup) {
      if (pdusRefsBackup) {
        logger.warn('  ⚠ dc_pdus FK references _dc_racks_backup (corrupted), fixing...');
      } else {
