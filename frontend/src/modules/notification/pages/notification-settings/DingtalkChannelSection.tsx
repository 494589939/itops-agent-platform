/**
 * 钉钉通知渠道 section widget（2026-07-21 拆分）
 *
 * 从原 NotificationSettings.tsx L441-505 抽出
...
export interface DingtalkChannelSectionProps {
    ...
    <div className="space-y-3 pb-4 border-b border-border last:border-0 last:pb-0">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-secondary">通过钉钉群机器人 Webhook 发送通知</p>
        </div>
        <ToggleSwitch
          checked={config.dingtalk_enabled}
          onChange={(b) => onChange({ dingtalk_enabled: b })}
          ariaLabel="启用钉钉"
        />
      </div>
