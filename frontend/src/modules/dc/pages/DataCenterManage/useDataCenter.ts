...
export default function useDataCenter() {
  ...
  const assignSlot = async () => {
    try {
      ...
      // 必须带上 device_type，否则后端 dc_rack_slots.device_type (NOT NULL) 写入失败
      // 设备类型从 availDevices 列表按 device_id 查得
      const device = availDevices.find(d => d.id === values.device_id);
      await api.post('/dc/slots', {
        ...values,
  ...
  const openMoveModal = (slot: Slot) => {
    setSelectedSlot(slot);
    moveForm.setFieldsValue({ rack_id: slot.rack_id, start_u: slot.start_u, end_u: slot.end_u, position_face: slot.position_face || 'front' });
